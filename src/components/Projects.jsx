...
const ProjectCard = ({ title, period, description, project_img, link, githubRepo }) => {
  ...
  return (
    ...
    >
      ...
      {/* Hover/Tap gradient */}
      <div className="absolute inset-0 bg-linear-to-t from-black via-black/60 to-transparent opacity-0 group-hover:opacity-100 group-[.is-tapped]:opacity-100 transition-opacity duration-500 ease-in-out"></div>
    
      {/* Content container */}
      <div className="absolute inset-0 flex flex-col justify-end p-6 text-white transition-all duration-500 ease-in-out transform translate-y-30 group-hover:translate-y-0 group-[.is-tapped]:translate-y-0">
        <h3 className="text-xl font-bold">{title}</h3>
        <p className="text-sm text-gray-400 italic mb-2">{period}</p>
        
        {/* Hidden content that appears on hover/tap */}
        <div className="opacity-0 group-hover:opacity-100 group-[.is-tapped]:opacity-100 transition-opacity duration-300 delay-200">
